...
#Hardcoded w48_256x256
cfg = {'PRETRAINED_LAYERS': ['conv1',
                             'bn1',
                             'conv2',
                             'bn2',
                             'layer1',
    ...
    def forward(self, x):
        ...
        for i in range(self.stage4_cfg['NUM_BRANCHES']):
            if self.transition3[i] is not None:
                ...
                x_list.append(y_list[i])
        x_final = self.stage4(x_list)

        x1 = self.upsample_stage_2(x_final[1])
        x2 = self.upsample_stage_3(x_final[2])
        x3 = self.upsample_stage_4(x_final[3])
        x = torch.cat([x_final[0], x1, x2, x3], 1)
        return x, x_final[3]
...
def get_pose_net(**kwargs):
    ...
    # model.init_weights("pose_hrnet_w48_256x192.pth")
    return model
